<x-layouts.app title="Tugas - Aviona Sync">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Tugas</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Kelola tugas, deadline, dan kegiatanmu.</p>
        </div>
    ...
    <div x-data="{ showFilters: {{ request()->anyFilled(['q', 'kategori', 'status', 'prioritas', 'source', 'tanggal_mulai', 'tanggal_selesai', 'sort']) ? 'true' : 'false' }} }" class="space-y-4">
        <div class="flex justify-start">
            <button @click="showFilters = !showFilters" class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 shadow-sm transition hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none focus:ring-4 focus:ring-slate-100 dark:focus:ring-slate-900">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 text-slate-500 dark:text-slate-400">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-9.75 0h9.75" />
                </svg>
                <span>Filter & Urutkan</span>
                <svg :class="showFilters ? 'rotate-180' : ''" class="h-4 w-4 text-slate-400 dark:text-slate-500 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @endphp
                @if ($activeCount > 0)
                    <span class="ml-1 rounded-full bg-pink-100 dark:bg-pink-900/30 px-2 py-0.5 text-xs font-semibold text-pink-700 dark:text-pink-400">
                        {{ $activeCount }}
                    </span>
                @endif
            </button>
            ...
            <x-card>
                <form method="GET" action="{{ route('jadwal-kegiatan.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <div class="xl:col-span-2">
                        <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Cari</label>
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Sumber</label>
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Mulai Tanggal</label>
                        <x-input type="date" name="tanggal_mulai" value="{{ request('tanggal_mulai') }}" />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Sampai Tanggal</label>
                        <x-input type="date" name="tanggal_selesai" value="{{ request('tanggal_selesai') }}" />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Urutkan</label>
                        <x-select name="sort">
            ...
            <x-card>
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div class="space-y-3">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ $jadwal->judul }}</h2>
                            ...
                            @if ($jadwal->course_name)
                                <p class="mt-0.5 text-xs text-slate-400 dark:text-slate-500">{{ $jadwal->course_name }}</p>
                            @endif
                            @if ($jadwal->lokasi_atau_link)
                                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400 break-all">{{ $jadwal->lokasi_atau_link }}</p>
                            @endif
                            @if ($jadwal->countdown_text)
                                <p class="mt-2 text-sm font-medium {{ $jadwal->isOverdue() ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400' }}">
                    ...
                    <div class="flex flex-wrap gap-3 lg:justify-end">
                        ...
                        <x-modal message="Tugas yang dihapus tidak bisa dikembalikan.">
                            <x-slot:trigger>
                                <button type="button" class="inline-flex items-center justify-center rounded-2xl bg-red-50 dark:bg-red-950/30 px-4 py-2.5 text-sm font-semibold text-red-700 dark:text-red-400 transition hover:bg-red-100 dark:hover:bg-red-900/30">Hapus</button>
                            </x-slot:trigger>
