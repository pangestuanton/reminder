<x-layouts.guest title="Masuk - Aviona Sync">
    <div class="w-full rounded-[2rem] bg-white dark:bg-slate-900 p-6 shadow-card border border-slate-100 dark:border-slate-800/80 sm:p-8">
        <div class="mb-8 text-center">
            ...
            <h2 class="mt-2 text-lg font-semibold text-slate-800 dark:text-slate-200">Masuk ke akunmu</h2>
            <p class="mt-2 text-sm leading-relaxed text-slate-500 dark:text-slate-400">Kelola jadwal kuliah, tenggat tugas, dan pengingat penting dalam satu dashboard yang rapi.</p>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="relative">
                ...
                <div class="relative flex justify-center text-sm">
                    <span class="bg-white dark:bg-slate-900 px-3 text-slate-400 dark:text-slate-500">atau masuk dengan email</span>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('login.store') }}" class="mt-6 space-y-5">
            ...
            <label class="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400">
                <input type="checkbox" name="remember" class="h-4 w-4 rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-pink-600 focus:ring-pink-500 dark:focus:ring-offset-slate-900">
        ...
        <div class="mt-6 rounded-2xl bg-pink-50/30 dark:bg-pink-950/10 px-4 py-3 text-center text-sm text-slate-500 dark:text-slate-400">
            Belum punya akun?
            <a href="{{ route('register') }}" class="font-semibold text-pink-600 hover:text-pink-700 dark:text-pink-400 dark:hover:text-pink-300 transition">Daftar sekarang</a>
        </div>
    </div>
</x-layouts.guest>
