<x-layouts.app title="Dashboard - Aviona Sync">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <x-card>
            ...
            <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-slate-100">
                <div class="h-full rounded-full bg-blue-600 transition-all" style="width: {{ $analytics['percentage'] }}%"></div>
            </div>
            <p class="mt-2 text-xs text-slate-500">{{ $analytics['completed'] }} dari {{ $analytics['total'] }} tugas selesai</p>
        </x-card>

        <x-card>
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-semibold text-slate-900">Agenda Hari Ini</h3>
                    <p class="mt-1 text-xs text-slate-500">{{ $todayClasses->count() }} kelas, {{ $todayEvents->count() }} acara</p>
                </div>
                <a href="{{ route('calendar.index') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-700">Lihat</a>
            </div>
        </x-card>
        ...
        <x-card>
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-semibold text-slate-900">Integrasi</h3>
                    <p class="mt-1 text-xs text-slate-500">{{ auth()->user()->hasClassroomAccess() ? 'Classroom aktif' : 'Classroom tidak terhubung' }}</p>
                </div>
                <a href="{{ route('integrations.index') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-700">Kelola</a>
            </div>
        </x-card>
    </div>
    ...
    @if ($todayClasses->isNotEmpty())
        <x-card>
            <h3 class="text-lg font-semibold text-slate-900">Kuliah Hari Ini</h3>
            <div class="mt-4 space-y-3">
                @foreach ($todayClasses as $class)
                    <div class="flex items-center gap-4 rounded-2xl bg-blue-50/50 p-4 transition hover:bg-blue-50">
                        ...
                        <div class="flex-1">
                            <h4 class="font-semibold text-slate-900">{{ $class->mata_kuliah }}</h4>
    ...
    @if ($todayEvents->isNotEmpty())
        <x-card>
            <h3 class="text-lg font-semibold text-slate-900">Acara Kalender</h3>
            <div class="mt-4 space-y-3">
                @foreach ($todayEvents as $event)
                    <div class="flex items-center gap-4 rounded-2xl bg-amber-50/50 p-4 transition hover:bg-amber-50">
                        <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-100 text-sm font-bold text-amber-700">
                            📅
                        </div>
                        <div class="flex-1">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div class="space-y-6">
                <x-card>
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            @else
                                <p class="mt-2 text-sm text-slate-500">Belum ada tugas mendatang.</p>
                            @endif
                        </div>
                        @if ($nearest)
                ...
                <x-card>
                    <div class="mb-4 flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-slate-900">Tugas Berikutnya</h3>
                        <a href="{{ route('jadwal-kegiatan.index') }}" class="text-sm font-semibold text-blue-600 hover:text-blue-700">Lihat semua</a>
                    </div>
                    <div class="space-y-4">
                        @forelse ($upcoming as $jadwal)
                            <div class="rounded-2xl bg-blue-50/30 p-4 transition hover:bg-blue-50/60">
                        ...
                        @empty
                            <p class="text-sm text-slate-500">Belum ada tugas terdekat.</p>
                        @endforelse
                    </div>
                </x-card>
            </div>
            ...
            <div class="space-y-6">
                <x-card>
                    ...
                    <p class="mt-1 text-sm text-slate-500">Tugas yang membutuhkan perhatianmu dalam 7 hari ke depan.</p>
                    <div class="mt-5 space-y-4">
                        @forelse ($urgentList as $jadwal)
                            <div class="rounded-2xl bg-yellow-50 p-4 transition hover:bg-yellow-100/60">
                ...
                @if ($recentNotifications->isNotEmpty())
                    <x-card>
                        <h3 class="text-lg font-semibold text-slate-900">Notifikasi Terakhir</h3>
                        <div class="mt-4 space-y-3">
                            @foreach ($recentNotifications as $log)
                                <div class="flex items-center gap-3 text-sm">
                                    <span class="h-2 w-2 rounded-full {{ $log->status === 'sent' ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                                    <span class="text-slate-600">{{ ucfirst(str_replace('_', ' ', $log->notification_type)) }}</span>
                                    <span class="ml-auto text-xs text-slate-400">{{ $log->sent_at?->diffForHumans() }}</span>
                                </div>
                            @endforeach
                        </div>
                    </x-card>
